...
public class GameAPI {
    ...
    @OnClose
    public void onClose(){
        System.out.println("close ！userId: "+this.userId);
        OnlineUserManager.getInstance().offline(this.userId);
    }

    @OnError
    public void onError(Session session , Throwable error){
        System.out.println("连接异常33！userId: "+this.userId);
        error.printStackTrace();
        OnlineUserManager.getInstance().offline(this.userId);
    }
    @OnMessage
    public void onMessage(String message , Session session) throws InterruptedException, IOException, SQLException {
        System.out.println("收到消息44 ！message: "+message+"userId :"+this.userId);
        //把message 按照JSON格式转换为对象
